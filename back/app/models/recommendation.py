...
class Constraints(BaseModel):
    ...
    tools: list[str] = Field(
        ...
        description="사용 가능한 조리 도구 목록"
    )
    exclude: list[str] = Field(
        default_factory=list,
        description="제외할 재료 목록 (알레르기 등)"
    )
...
class RecommendationCreate(BaseModel):
    ...
    ingredients: list[str] = Field(
        min_length=1,
        description="냉장고에 있는 재료 목록 (최소 1개 이상)"
    )
    constraints: Constraints = Field(
        default_factory=Constraints,
        description="조리 제약 조건"
    )
...
class Recipe(BaseModel):
    ...
    ingredients_total: list[str] = Field(
        description="레시피에 필요한 전체 재료 목록"
    )

    # 사용자가 입력한 재료(=보유)와 비교해 분리한 결과
    ingredients_have: list[str] = Field(
        description="사용자가 이미 보유한 재료"
    )
    ingredients_need: list[str] = Field(
        description="구매가 필요한 재료"
    )

    steps: list[str] = Field(
        description="조리 순서 (4-8단계)"
    )
    tips: list[str] = Field(
        default_factory=list,
        description="조리 팁"
...
class ShoppingItem(BaseModel):
    ...
    qty: float | int | None = Field(
        default=None,
        description="수량"
    )
    unit: str | None = Field(
        default=None,
        description="단위 (예: 개, 봉, ml, g)"
    )
    category: str | None = Field(
        default=None,
        description="카테고리 (예: 채소, 육류, 조미료)"
    )
...
class RecommendationResponse(BaseModel):
    ...
    id: str = Field(
        description="추천 ID (rec_로 시작하는 고유 식별자)"
    )
    created_at: datetime = Field(
        description="생성 일시"
    )
    recipes: list[Recipe] = Field(
        description="추천된 레시피 목록 (정확히 3개)",
        min_length=3,
        max_length=3
    )
